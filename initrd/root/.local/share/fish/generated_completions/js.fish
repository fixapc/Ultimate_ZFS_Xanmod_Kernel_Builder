...
complete -c js -l experimental-global-customevent -d 'Expose the CustomEvent on the global scope'
complete -c js -l experimental-global-webcrypto -d 'Expose the Web Crypto API on the global scope'
...
complete -c js -l experimental-network-imports -d 'Enable experimental support for loading modules using `import` over `https:`'
...
complete -c js -l trace-atomics-wait -d 'Print short summaries of calls to Atomics. wait()'
