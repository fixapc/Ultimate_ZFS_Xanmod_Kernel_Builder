...
complete -c nodejs -l experimental-global-customevent -d 'Expose the CustomEvent on the global scope'
complete -c nodejs -l experimental-global-webcrypto -d 'Expose the Web Crypto API on the global scope'
...
complete -c nodejs -l experimental-network-imports -d 'Enable experimental support for loading modules using `import` over `https:`'
...
complete -c nodejs -l trace-atomics-wait -d 'Print short summaries of calls to Atomics. wait()'
