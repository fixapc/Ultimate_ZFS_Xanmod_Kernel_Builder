...
complete -c llvm-objcopy -l change-start -l adjust-start -d 'Add <incr> to the programâs start address'
...
complete -c llvm-objcopy -l discard-locals -s X -d 'Remove local symbols starting with â. Lâ from the output'
